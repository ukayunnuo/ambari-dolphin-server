...
dolphin_env_path = dolphin_conf_dir + '/dolphinscheduler_env.sh'
...
dolphin_common_map_tmp = config['configurations'].get('dolphin-common', {})

Logger.info("Load dolphin-common config: %s" % dolphin_common_map_tmp)
# 确保 resource.storage.type 存在，否则使用默认值 HDFS
data_basedir_path = dolphin_common_map_tmp.get('data.basedir.path', '/tmp/dolphinscheduler')
...
dolphin_app_api_map['resource.storage.type'] = dolphin_common_map_tmp.get('resource.storage.type', 'HDFS')
...
dolphin_common_map.update(dolphin_common_map_tmp)
